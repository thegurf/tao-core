...
class tao_test_FileHelperTest extends TaoPhpUnitTestRunner
{
    ...
    {
        $this->sampleDir = __DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'samples';
        $this->tmpPath = sys_get_temp_dir();
        $this->envName = 'ROOT_DIR';
        $this->envPath = $this->tmpPath . DIRECTORY_SEPARATOR . $this->envName;
    }
    ...
    {
        $this->assertEquals(count(tao_helpers_File::scanDir($this->envPath, array(
            'recursive' => true
        ))), 23);
        $this->assertEquals(count(tao_helpers_File::scanDir($this->envPath, array(
            'only' => tao_helpers_File::$DIR,
            'recursive' => true
        ))), 3);
        $this->assertEquals(count(tao_helpers_File::scanDir($this->envPath, array(
    ...
    public function testIsIdentical()
    {
        ...
        $zip = new ZipArchive();
        $this->assertTrue($zip->open($this->sampleDir . DIRECTORY_SEPARATOR . 'fileHelper.zip'));
